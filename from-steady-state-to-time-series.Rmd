...
```{r}
getErrorYield<-function (vary, params, data_type = "catch", tol = 0.1,timetorun = 10) 
{
    gear_params(params)$catchability <-vary     
    ...
    pred <- log(output)
    dat <- log(species_params(params)$yield_observed)
    discrep <- pred - dat
...
rect_vary<-cbind(rep(0.2,num_species),rep(1.5,num_species))
set.seed(123)
...
plotYieldVsF(params,species="Cod",F_max = 1.0)
