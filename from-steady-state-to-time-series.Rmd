...
We will use the North Sea model parameters which includes time-averaged biomass and yield observations needed to carry out this example.
...
I repeated the below 3 times, which gives a total relative error of 0.918.
...
# lets start the model off with levels of fishing using the first year of the input values
...
Here, we are interested in examining the changes along side observations. Let's read in the observed landings for the North Sea and add these to our plot.
...
First let's just see what happens if we reset the reprdocution level directly for a species.
```{r}
paramsRepro <- setBevertonHolt(params2, reproduction_level = c("Cod" = 0.5))
...
Here we can see that reducing the reproduction level does have a big effect on when the stocks start collapsing, as expected. And having the values low doesn't seem consistent with the data.
...
respond to fishing this is an appropriate one to include, but only when Rmax is also being re-calibrated.
...
mortality parameters. Bayesian parameter uncertainty work has done this with Rmax (Spence et
al. 2016) and time-varying Fs (Spence et al 2021) using this North Sea
model. For example, we could use a logistic function to capture changes
in fishing mortality rates through time (Svuwalski et al. 2017). 
...
sim_optim<-fastOptim(params=params2,effort=effort,dat=yields_obs)

# We can alternatively do this with latin hypercube sampling:
num_species<-12
# num runs
num_iter=10
# set the upper and lower numbers
rect_vary<-cbind(rep(0.5,num_species),rep(0.9999,num_species))
set.seed(123)
parameters_new <- lhs(num_iter, rect_vary)

# we could also the pbapply function in parallel so it's faster, here just using as it has a nice progress bar (so you know how long you can go and take a break)

parameters_new$error<- pbapply(parameters_new,1,getErrorTime,params2,dat=yields_obs,effort)
...
paramsTEST<-setBevertonHolt(paramsTEST,reproduction_level = vary)

simt5<- project(paramsTEST, effort = effort)

plotYield(simt5) +    facet_wrap(~Species,scales="free_y")+
