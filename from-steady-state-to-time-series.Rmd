...
For this example, I will use the North Sea model, but with a focus on developing a workflow for any mizer model that has catch and effort (or fishing mortality rate) time-series and at least time averaged biomass observations.
...
and [mizerHowTo packages](https://sizespectrum.org/mizerExperimental/). Therefore we will always want to make sure we
...
library(lhs)
library(tgp)
source("calibration_functions.R") # move to howTO

```

The North Sea model parameters which includes time-averaged biomass and
yield observations needed to carry out this example. Except for sandeel, all of the biomasses (grams) are from survey data, corrected for catchability, and averaged over 1985-1995 for the whole area of the North Sea. For sandeel, stock assessment etsimates of spawner biomass was used. For catches, discards are included, with the exception of Gurnard and Dab which, along with catchability values, are the same as in Blanchard et al. 2014.
...
# here we are looking at the parameter space below the current values, as the yields look too high from the model
...
We can take a look at how the overall  error changes as the catchability for all species are simultaneously changed.
```{r}
...
We can then find the parameter set with the lowest error, and repeat as needed.
...
Let's plug the estimates from our third try back into the model.
...
plotYieldObservedVsModel(params)
```
It's a bit of an improvement for yields. How will this affect our biomass fits?

```{r}
plotBiomassObservedVsModel(params)
```

An alternative approach to find these ballpark values is to optimize one species at a time (bit this does not guarantee a good fit because changing one species parameters affects the outputs of other species). But let's see how the different approaches compare.
...
Plug the single species estimates of catchability back into model.
...
This didn't really improve things. And both made the biomass fits worse.We can re-calibrate the biomass, now that we have new catchability parameters.
...
```
Re-calibrating biomass puts us back at the beginning in terms of capturing the time-averaged yields well. More data would be advantageous here to help us learn about the scope of these parameters.


# Using the time series data to estimate reproduction and catchability parameters

In addition to the catchabilities, we are uncertain about the reproduction_level values. What would the reproduction parameters need to be in order to fit the
...
```{r}
params2 <- setBevertonHolt(params, reproduction_level = 0.09)
...
```{r,code_folding=TRUE,eval=F}
sample_params2<-function(vary=c(as.numeric(getReproductionLevel(params2)),gear_params(params2)$catchability),species=NULL) { # set up workers
  noCores <- parallel::detectCores() - 1 # keep some spare core
  cl <- parallel::makeCluster(noCores, setup_timeout = 0.5)
  setDefaultCluster(cl = cl)
  clusterExport(cl, varlist = "cl",envir=environment())
...
num_species<-length(whichspecies)
# num runs
num_iter=10000
# set the upper and lower bounds for each variable
 upper<-vary*1.1
 lower<-vary*0.6
 rect_vary<-cbind(lower,upper)
#rect_vary<-cbind(rep(0.01,num_species*2),rep(0.9999,num_species*2))
set.seed(123)
parameters_new <- data.frame(lhs(num_iter, rect_vary))
names(parameters_new)<-row.names(species_params(params))[whichspecies]

parameters_new$error<-pbapply(parameters_new,1,getErrorTime,params2,dat=yields_obs,effort,species=species,cl=cl)

return(parameters_new)

stopCluster(cl)
}
parameters_new<-sample_params2(vary=c(0.9,0.9),species="Cod")
#save error
#saveRDS(parameters_new,"lhs_paramspace.RDS")
...
```{r,code_folding=TRUE,eval=F}
params_lhs<-params2
params_lhs<-setBevertonHolt(params_lhs, reproduction_level = unlist(result[1:12]))

gear_params(params_lhs)$catchability<-unlist(result[13:24])
params_lhs<- projectToSteady(params_lhs, effort = effort[1,])
simt_lhs<- project(params_lhs, effort = effort)

plotYield(simt_lhs) +    facet_wrap(~Species,scales = "free")+
geom_point(data=filter(yields_obs),aes(x = Year, y = (Yield), colour = Species),size=0.1)

getReproductionLevel(params_lhs)


# check error and compare with original error
(new<-getErrorTime(vary=unlist(result[1:24]),params=params_lhs,effort,yields_obs,variable = "reproduction_level"))
...
```{r,code_folding=TRUE,eval=F}
quickOptim<-function(params=params2) {
  # set up workers
  noCores <- parallel::detectCores() - 1 # keep some spare core
  cl <- parallel::makeCluster(noCores, setup_timeout = 0.5)
  setDefaultCluster(cl = cl)
...
vary<-c(as.numeric(getReproductionLevel(params2)),gear_params(params2)$catchability)

# you can keep track of how long this takes
library(tictoc)
tic()
optim_result <- optimParallel::optimParallel(par=vary,getErrorTime,params=params2, effort =effort,dat = yields_obs, method   ="L-BFGS-B", lower=c(rep(0.01,(dim(params2@species_params)[1])*2)), upper= c(rep(0.999,(dim(params2@species_params)[1])*2)),                                parallel=list(loginfo=TRUE, forward=TRUE))

stopCluster(cl)
toc()
# 3809.443 sec elapsed (using 7 cores on a Mac Book Pro) 
}
optim_results2<-quickOptim(simt_optim@params)
saveRDS(optim_results2,"optim_results2.RDS")
...
plotBiomassObservedVsModel(params_optim)
