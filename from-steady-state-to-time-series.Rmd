...
#remotes::install_github("sizespectrum/mizerMR",force=TRUE)
...
```
The model is overestimating the yields, likely to do with how the catchability coefficients have been parameterised from stock assessment estimates of fishing mortality rates that capture all age ranges. Let's estimate the catchability based on these time averaged yields. We can do this by writing an "error function" that allows us to optimise (find the minimum value of the error) by adjusting the values of a parameter, in this case catchability. Later on, we will apply time series expressing changes in F (from stock assessments) relative to the initial catchability values.
...
getErrorYield<-function (vary, params, data_type = "catch", tol = 0.1,timetorun = 10,species=NULL) 
{
  ...
  if (is.null(species)) {
    ...
    model <- getYield(sim)[timetorun, ]
    observed <- species_params(params)$yield_observed
...
pbapply(parameters_new,1,getErrorYield,params)



for (i in 1:12){
speciesl<-row.names(species_params(params))[i]
out<-optimise(getErrorYield,c(0,1),params,species=speciesl,tol=0.1)
gear_params(params)[gear_params(params)$species=="Dab",]$catchability<-out$minimum
params<-steady(params)
}
```
...
rect_vary<-cbind(vary*0.8,vary*1.2)
set.seed(123)
...
#save error
parameters_new<-sample_params(vary=gear_params(params)$catchability)
...
```{r}
gear_params(params)$catchability<-as.numeric(result[1:12])
params2<-steady(params)
plotYieldObservedVsModel(params2,ratio=F)
# check biomass again
plotBiomassObservedVsModel(params2,ratio=F)
```
...
plotYieldVsF(params,species="Sprat")
```
