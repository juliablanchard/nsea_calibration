...
getErrorTime <- function(vary,params,dat,env=state,tol = 0.1) {
  ...
  effort<-simt@effort
  species_params(params)$R_max[1:12]<-10^vary[1:12]
  species_params(params)$erepro[1:12]<-vary[13:24]
  resource_params(params)$kappa<-10^vary[25]
  resource_params(params)$r_pp<-vary[26]
  ...
  #run time-varying effort model through time with new erepro
  params <- steady(params)
  
  newsimt <- project(params, effort = effort)
  # get biomass through time
  biomass <- sweep(newsimt@n, 3, newsimt@params@w*newsimt@params@dw, "*")
...
plotFittedTime<-function(simt=simt,obsy=obsy,allSpecies=T,plotSpecies=NULL,startyr=1947,endyr=2019){
